...
void CFDDiscovery::Write(std::ofstream& file) const {
    fDb.write(file);
}

void CFDDiscovery::PrintAttrs() {
    std::cout << fGens.GetGenMap().size() << " " << fStore.size() << ' ' << fTidStore.size()
              << ' ' << fAllAttrs.size() << ' ' << fItemsLayer.size() << ' ' << fFreeMap.size() << fFreeItemsets.size() << ' ' << fRules.size() << ' ' << fAttOrder.size();
}
    ...
    for (int ix = items.size()-1; ix >= 0; ix--) {
        ...
        if (suffix.size()) {
            std::sort(suffix.begin(), suffix.end(),
                      [this](const MinerNode<PartitionTidList>& a, const MinerNode<PartitionTidList>& b) {
                return a.fTids.fNrSets < b.fTids.fNrSets;
            });
...
int CFDDiscovery::getPartitionError(const SimpleTidList& pids,
                                    const std::vector<std::pair<Itemset, std::vector<int>>>& partitions) {
    int res = 0;
    for (int p : pids) {
        int max = 0;
        int total = 0;
...
std::vector<MinerNode<PartitionTidList> > CFDDiscovery::getPartitionSingletons(const SimpleTidList& tids,
                                                                               const Itemset& attrs) {
    std::vector<std::vector<SimpleTidList> > partitions(fDb.nrAttrs());
    std::unordered_map<int, std::pair<int,int> > attrIndices;
    for (int a : attrs) {
